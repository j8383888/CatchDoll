...
module catchDoll {
	export class GameObjectPool {
		 ...
		 * 一级缓存用于缓存只被用了一次的资源
		 */
		private _objectFirstPool: SimpleMap<GameObject[]> = new SimpleMap<GameObject[]>();
		/**
		 * 二级缓存用于缓存经常使用的资源，从资源池拿去先从当前资源池拿取
		 */
		private _objectSecondPool: SimpleMap<GameObject[]> = new SimpleMap<GameObject[]>();
		 ...
		 * 尝试从对象池获取对象
		 */
		public tryGetGameObjInPool(sign: GAMEOBJECT_SIGN): GameObject {
		...
		public recoverGameObject(gameObj: GameObject): void {
			// this._calCacheNum();
			var gameObjFirstAry: Array<GameObject> = null;
			var gameObjSecAry: Array<GameObject> = null
			...
			if (gameObj.refCount >= 2) {
				...
				if (gameObjSecAry == null) {
					gameObjSecAry = new Array<GameObject>();
			...
			/*引用计数为1时 进入一级缓存*/
				...
				if (gameObjFirstAry == null) {
					gameObjFirstAry = new Array<GameObject>();
		...
		private _clearPool(cacheMap: SimpleMap<GameObject[]>) {
			...
			for (var i: number = 0; i < len; i++) {
				...
				let copy: GameObject[] = gameObjAry.slice();
				let copyLen: number = copy.length;
				for (var j: number = 0; j < copyLen; j++) {
					let gameObj: GameObject = copy[j]
